...
const cn = {
  ...
  Settings: {
    ...
    AutoBackup: {
      ...
      Enabled: "启用自动备份",
      Interval: "备份间隔（小时）",
      IntervalDesc: (hours: number) => `每 ${hours} 小时备份一次`,
      MaxBackups: "最多保留备份数",
      MaxBackupsDesc: (count: number) => `最多保留 ${count} 个备份文件`,
      BackupPath: "备份路径",
      BackupPathDesc: "桌面端：备份文件保存位置",
      SelectPath: "选择文件夹",
      DefaultPath: "使用默认路径",
      Status: "备份状态",
      LastBackup: "上次备份",
      Never: "从未备份",
      BackupCount: (count: number) => `${count} 个备份`,
      TotalSize: (size: string) => `总大小：${size}`,
      BackupNow: "立即备份",
      ViewBackups: "查看备份",
      ClearAll: "清空所有备份",
      ClearAllConfirm: "确认删除所有自动备份吗？",
